...
impl IndexWorker {
    ...
    async fn prepare(
        ...
        let history = {
            ...
            // Both sides of the archive pipeline are async, timeout- and
            // kill-guarded; keep the mirror locked until they finish.
...
async fn commit_probe_succeeds_before(
    mirror: &Path,
    timeout: Duration,
    status: impl std::future::Future<Output = std::io::Result<std::process::ExitStatus>>,
) -> bool {
...
async fn extract_tree(mirror: &Path, commit: &str, dest: &Path) -> anyhow::Result<()> {
    tokio::time::timeout(
...
async fn extract_tree_pipeline(mirror: &Path, commit: &str, dest: &Path) -> anyhow::Result<()> {
