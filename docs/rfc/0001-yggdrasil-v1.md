...
- Operational surface (non-Verb): `GET /healthz` — unauthenticated readiness, per dependency (control plane, object storage); `GET /v1/status` — version, uptime, indexed-repo count, behind auth, surfaced as `yg status [--json]`. Every route except `/healthz` requires a token, including unmatched paths. Default listen address `127.0.0.1:7311`. Until Admin-issued Member tokens land (`members`/`tokens`, §2), the server boots from a bootstrap Admin token in `YG_BOOTSTRAP_TOKEN` — held in memory, never stored; the `ygt_` prefix is convention, not enforced. The richer `yg admin status` (§3) arrives with the admin surface.
